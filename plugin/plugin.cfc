...
component persistent="false" accessors="true" output="false" extends="mura.plugin.plugincfc" {
	...
	public void function install() {
		...
		if (dbCheckTablesResults.getResult().recordcount eq 0) {
			...
			var dbCreateTable = new query();
			dbCreateTable.setDatasource("#application.configBean.getDatasource()#");
			dbCreateTable.setSQL("CREATE TABLE `p_Auction_Items` ( `TContent_ID` int(11) NOT NULL AUTO_INCREMENT, `Item_Title` varchar(64) DEFAULT NULL, `Organization_ID` int(11) NOT NULL, `Location_ID` int(128) DEFAULT NULL, `Item_AtParentLocation` bit(1) DEFAULT NULL, `Site_ID` tinytext NOT NULL, `Item_ModelNumber` tinytext NOT NULL, `Item_Description` tinytext NOT NULL, `Item_Condition` tinytext NOT NULL, `Item_UPC` tinytext, `Item_YearsInService` tinytext, `Item_InfoWebsite` tinytext, `AssetTag_Number` tinytext, `Item_WebDirectory` tinytext, `Item_PrimaryPhoto` tinytext, `Item_SecondaryPhoto` tinytext, `Item_TertiaryPhoto` tinytext, `Auction_StartDate` datetime NOT NULL, `Auction_EndDate` datetime NOT NULL, `Auction_Category` int(11) NOT NULL, `Auction_Type` varchar(7) NOT NULL DEFAULT '0', `Starting_Price` double(15,2) DEFAULT NULL, `Current_Bid` double(15,2) DEFAULT NULL, `HighestBid_UserID` tinytext, `dateCreated` datetime NOT NULL, `lastUpdated` timestamp NULL DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP, `lastUpdateBy` tinytext NOT NULL, `lastUpdateByID` tinytext NOT NULL, `Active` bit(1) DEFAULT b'0', `Auction_Won` bit(1) DEFAULT NULL, `Auction_WonDate` datetime DEFAULT NULL, `Auction_WonUserID` tinytext, PRIMARY KEY (`TContent_ID`) ) ENGINE=MyISAM AUTO_INCREMENT=1 DEFAULT CHARSET=latin1;");
			var dbCreateTableResults = dbCreateTable.execute();
		} else {
			// Database Table Exists, We must Drop it to create it again
			var dbDropTable = new query();
		...
		if (dbCheckTablesResults.getResult().recordcount eq 0) {
			...
			var dbCreateTable = new query();
			dbCreateTable.setDatasource("#application.configBean.getDatasource()#");
			dbCreateTable.setSQL("CREATE TABLE `p_Auction_SiteConfig` ( `TContent_ID` int(11) NOT NULL AUTO_INCREMENT, `Site_ID` tinytext NOT NULL, `DateCreated` datetime NOT NULL, `lastUpdateBy` varchar(35) NOT NULL, `lastUpdated` datetime NOT NULL, `SellerPercentageFee` decimal(7,3) DEFAULT NULL, `ProcessPayments_Stripe` bit(1) NOT NULL DEFAULT b'0', `Stripe_TestMode` bit(1) NOT NULL DEFAULT b'1', `Stripe_TestAPIKey` tinytext, `Stripe_LiveAPIKey` tinytext, PRIMARY KEY (`TContent_ID`) ) ENGINE=InnoDB AUTO_INCREMENT=1 DEFAULT CHARSET=latin1;");
			var dbCreateTableResults = dbCreateTable.execute();
		} else {
			// Database Table Exists, We must Drop it to create it again
			var dbDropTable = new query();
